...
class CompareModelGenerationTest extends TestCase
{
     ...
     */
    // Disabled due to external service dependencies
    // #[Test]
    public function disabled_it_compares_pk_generation_quality_across_models()
    {
        ...
        $testAdvisor = Advisor::factory()->create([
            ...
            'communication_style_description' => 'Direct and data-driven communication',
        ]);
    ...
    private function generatePKWithModel(Advisor $advisor, string $model, bool $useWebSearch): string
    {
        ...
        if ($useWebSearch) {
            // For deep research, return mock response
            $llmService->shouldReceive('generateText')
                ->andReturn(str_repeat('Generated PK content for '.$model.' with deep research. ', 50));
        } else {
            // For standard models, return mock response
            $llmService->shouldReceive('generateText')
                ->andReturn(str_repeat('Generated PK content for '.$model.' with structured output. ', 50));
        }
        ...
        // Generate PK
        $result = $service->generateAdvisor($advisor);
