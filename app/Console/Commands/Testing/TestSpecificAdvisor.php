...
class TestSpecificAdvisor extends Command
{
    protected $signature = 'testing:advisor 
                            {advisor : Advisor key (e.g., bogusky, henderson, hormozi, halbert)}
    ...
    public function handle(
        ...
        if (!$advisor) {
            ...
            Advisor::pluck('key')->each(fn($key) => $this->line("  - {$key}"));
        ...
        $outputDir = $this->option('output-dir') 
            ?? "advisor-tests/experiments/{$advisor->key}/" . now()->format('Y-m-d_H-i-s');
    ...
    private function checkForCommonIssues(array $result): void
    {
        ...
        if ($advisor && in_array($advisor->key, ['henderson', 'halbert'])) {
