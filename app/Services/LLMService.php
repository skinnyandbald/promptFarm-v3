...
class LLMService
{
    ...
    {
        $model = $options['model'] ?? $this->config['model'] ?? 'o4-mini-deep-research';
        
        // Check if this is a deep research model (o3 or o4)
        if (str_contains($model, 'deep-research')) {
            return $this->generateWithDeepResearch($prompt, $options);
        }
    ...
    protected function generateWithDeepResearch(string $prompt, array $options = []): string
    {
        ...
        $payload = [
            ...
            'text' => [
                'format' => ['type' => 'text'],
                'verbosity' => 'medium'  // o3-deep-research supports medium verbosity
            ]
        ];
